...
export const getWeather = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const byDay = new Map<string, { hi: number; lo: number; ids: number[]; date: Date }>();
    for (const entry of fc.list) {
      const d = new Date(entry.dt * 1000);
      const dkey = d.toISOString().slice(0, 10);
      if (dkey === today) continue;
      const slot = byDay.get(dkey) ?? { hi: -Infinity, lo: Infinity, ids: [], date: d };
    ...
    const forecast = Array.from(byDay.values())
      ...
      .map((s, i) => ({
        ...
        kind: mapOwm(s.ids[Math.floor(s.ids.length / 2)] ?? 800),
        hi: s.hi,
        lo: s.lo,
      }));
